...
class Chunk(Base):
    """A chunk of text (sub-page-sized) used for embedding and retrieval.

    `vector_id` is a plain autoincrement integer — separate from the string
    UUID primary key — because FAISS's IndexIDMap requires int64 ids. It's
    the join key between this row and its vector in the FAISS index.
    """

    __tablename__ = "chunks"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=_uuid)
    vector_id: Mapped[int] = mapped_column(Integer, unique=True, nullable=False)
    paper_id: Mapped[str] = mapped_column(String, ForeignKey("papers.id"), nullable=False)
