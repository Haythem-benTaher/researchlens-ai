...
from app.services.retrieval_service import index_paper
...
def ingest_pdf(db: Session, *, original_filename: str, file_path: Path, file_size_bytes: int) -> Paper:
    ...
    # Chunk + embed + index for semantic search (Milestone 2). Kept as a
    # best-effort step: if it fails (e.g. embedding model unavailable),
    # the paper is still usable for browsing/reading, just not searchable
    # yet — status stays "ready" rather than failing the whole upload.
    try:
        index_paper(db, paper.id)
    except Exception as exc:
        paper.error_message = f"Indexing for search failed: {exc}"
        db.commit()
        db.refresh(paper)
